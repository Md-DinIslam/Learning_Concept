...
bool isSafe(int i, int j, int num) {
    ...
    for (int x = sx; x < sx + 3; ++x) {
        for (int y = sy; y < sy + 3; ++y) {
            if (sudo[x][y] == num) return false;
        }
    }
    return true;
}
bool sudoSolver(int i = 0, int j = 0) {
    // Base Case....
    if (i == 9) {
        printAll();
        return true;
    ...
    // Recursion Case.....
    if (j == 9) {
        return sudoSolver(i + 1, 0);
    }
    // Skip the prefill cell
    if (sudo[i][j]) {
        return sudoSolver(i, j + 1);
    }
    // Try out all possible.....
    for (int num = 1; num <= 9; ++num) {
        // whether can we place a number or not.....
        if (isSafe(i, j, num)) {
            sudo[i][j] = num;
            bool success = sudoSolver(i, j + 1);
            if (success) {
                return true;
            }
        }
